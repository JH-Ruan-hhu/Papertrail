...
const source=path.resolve(__dirname,'..');const stage=fs.mkdtempSync(path.join(os.tmpdir(),'yanji-release-build-'));console.log('RELEASE_BUILD_STAGE '+stage);
...
fs.cpSync(path.join(stage,'outputs'),path.join(source,'outputs'),{recursive:true});console.log('YANJI_RELEASE_BUILD_OK '+path.join(source,'outputs'));
