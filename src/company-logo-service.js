...
async function fetchPublic(value, { lookup = dns.lookup, deadline = Date.now()+10000, redirects = 0 } = {}) {
  ...
  const result=await new Promise((resolve,reject)=>{
    const request=(url.protocol==='https:'?https:http).get(url,{
      headers:{'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/150.0.0.0 Safari/537.36','Referer':url.origin+'/', 'Accept':'text/html,image/png,image/jpeg,image/webp,image/x-icon'},
      ...
      if(response.statusCode!==200){response.resume();reject(new Error('Logo 服务返回 HTTP '+response.statusCode+'，请检查官网或图标链接。'));return;}
...
function candidates(html, base) {
  ...
  for(const container of html.matchAll(/<[^>]+(?:class|id)=["'][^"']*(?:logo|brand)[^"']*["'][^>]*>([\s\S]{0,800}?<img\b[^>]*>)/gi)){const match=container[1].match(/(?:data-src|src)=["']([^"']+)["']/i);if(match)add(match[1],'official_site',2);}
...
class CompanyLogoService {
  constructor({store,directory,nativeImage,request=fetchPublic,svgToPng=rasterizeSvg,resolutionTimeoutMs=25000,onChanged=()=>{}}) {Object.assign(this,{store,directory,nativeImage,request,svgToPng,resolutionTimeoutMs,onChanged});this.pending=new Map();this.queue=[];this.running=0;}
  ...
  normalize(body,type){if(!Buffer.isBuffer(body)||body.length<8||body.length>MAX_BYTES)throw new Error('图片大小无效。');const signature=imageSignature(body);const allowed={png:['image/png','image/x-icon','image/vnd.microsoft.icon'],jpeg:['image/jpeg','image/x-icon','image/vnd.microsoft.icon'],webp:['image/webp','image/x-icon','image/vnd.microsoft.icon'],ico:['image/x-icon','image/vnd.microsoft.icon']};if(!signature||!(allowed[signature].includes(type)||['application/octet-stream','binary/octet-stream',''].includes(type)))throw new Error('图片格式或文件头无效。');const image=decodeIcon(body,this.nativeImage);if(image.isEmpty())throw new Error('图片解码失败。');const {width,height}=image.getSize();if(!width||!height||width*height>40000000)throw new Error('图片尺寸无效。');return (Math.max(width,height)>512?image.resize(width>=height?{width:512}:{height:512}):image).toPNG();}
  ...
  ensure(id,force=false){const c=this.company(id);const cached=this.snapshot(id);if(c.logoSource==='manual'||(!force&&Date.now()-Date.parse(c.logoLastCheckedAt||0)<(cached.logoData?TTL:86400000)))return Promise.resolve(cached);if(this.pending.has(id)){if(force){const index=this.queue.findIndex(item=>item.id===id);if(index>0)this.queue.unshift(...this.queue.splice(index,1));}return this.pending.get(id);}const work=new Promise(resolve=>force?this.queue.unshift({id,resolve}):this.queue.push({id,resolve}));this.pending.set(id,work);this.drain();return work;}
  ...
  async resolve(id){const deadline=Date.now()+this.resolutionTimeoutMs;const request=async url=>{const remaining=deadline-Date.now();if(remaining<=0)throw Error('图标识别超时，请稍后重试或选择图片。');let timer;try{return await Promise.race([this.request(url,{deadline:Math.min(deadline,Date.now()+10000)}),new Promise((_,reject)=>{timer=setTimeout(()=>reject(Error('图标识别超时，请稍后重试或选择图片。')),Math.min(remaining,10000));})]);}finally{clearTimeout(timer);}};const original=this.company(id);this.update(id,{logoStatus:'pending'});try{
    const jobs=this.store.listJobApplications().filter(j=>j.companyId===id);const sites=original.website?[]:[...new Set(jobs.map(j=>j.sourceUrl).filter(Boolean))].slice(0,3);
    const options=[],sourceIcons=[],resources=[];let website=original.website,lastError='';
    for(const site of sites){try{const page=await request(site);if(page.type==='text/html'){const html=page.body.toString('utf8');options.push(...candidates(html,page.url||site).filter(c=>c.source==='job_source'));sourceIcons.push(...candidates(html,page.url||site).filter(c=>c.source==='favicon'),{url:new URL('/favicon.ico',page.url||site).href,source:'favicon'});website ||= employerWebsite(html,page.url||site);if(!website){const title=html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1]||'';const name=String(original.name||'').replace(/股份有限公司|有限公司|科技有限公司/g,'').trim();const host=new URL(page.url||site).hostname;if(name.length>=2&&title.includes(name)&&!/(zhipin|liepin|51job|zhaopin|yingjiesheng|nowcoder|mokahr|feishu|metahr|myworkdayjobs)\./i.test(host)){website=new URL(page.url||site).origin;options.push(...candidates(html,page.url||site));}}}}catch{}}
    if(website){if(!original.website)this.update(id,{website,domain:new URL(website).hostname});try{const page=await request(website);if(page.type==='text/html'){const html=page.body.toString('utf8'),base=page.url||website;options.push(...candidates(html,base));for(const tag of html.matchAll(/<(?:script|link)\b[^>]*>/gi)){const match=tag[0].match(/(?:src|href)=["']([^"']+)["']/i);if(!match||!/(?:stylesheet|type=["']module)/i.test(tag[0]))continue;try{const url=safeUrl(new URL(match[1],base).href);if(url.origin===new URL(base).origin)resources.push(url.href);}catch{}}}else if(imageSignature(page.body)||page.type==='image/svg+xml')options.push({url:page.url||website,source:'official_site'});}catch(error){lastError=error.message;}options.push({url:new URL('/favicon.ico',website).href,source:'favicon'});}
    options.push(...sourceIcons);
    for(const option of options.filter((item,index,list)=>list.findIndex(other=>other.url===item.url)===index)){try{const image=await request(option.url);if(this.company(id).logoSource==='manual')return this.snapshot(id);if(image.type==='image/svg+xml'){const png=await this.svgToPng(image.body);if(this.company(id).logoSource==='manual')return this.snapshot(id);return this.write(id,png,'image/png',option.source,image.url||option.url);}return this.write(id,image.body,image.type,option.source,image.url||option.url);}catch(error){lastError=error.message;}}
    for(const resource of [...new Set(resources)].slice(0,3)){if(Date.now()>=deadline)break;try{const page=await request(resource),text=page.body.toString('utf8');const urls=[...text.matchAll(/["'(]([^"'()\s]*logo[^"'()\s]*\.(?:png|svg|webp|jpe?g)(?:\?[^"'()\s]*)?)["')]/gi)].map(m=>m[1]).sort((a,b)=>Number(!/nav[_-]?logo/i.test(a))-Number(!/nav[_-]?logo/i.test(b)));for(const value of [...new Set(urls)].slice(0,4)){try{const url=safeUrl(new URL(value,page.url||resource).href).href,image=await request(url);if(this.company(id).logoSource==='manual')return this.snapshot(id);const png=image.type==='image/svg+xml'?await this.svgToPng(image.body):image.body;return this.write(id,png,image.type==='image/svg+xml'?'image/png':image.type,'official_site',url);}catch(error){lastError=error.message;}}}catch(error){lastError=error.message;}}
    if(Date.now()>=deadline)throw Error('图标识别超时，请稍后重试或选择图片。');
