...
function careerTableRow(job) {
 ...
 return `<article class="job-position${job.status==='closed'?' is-closed':''}${careerPinned(job)?' is-pinned':''}" data-job-id="${id}" tabindex="0" aria-label="${wbEscape(job.company)} ${wbEscape(job.role)}"><div class="job-position-main"><div class="job-company-cell"><div class="job-company-name"><strong>${wbEscape(job.company)}</strong></div><span class="career-row-role"><span class="career-role-text">${wbEscape(job.role)}</span>${careerFavorite(job)}</span></div><div>${wbEscape(job.city||job.location||'—')}</div><div>${careerTypeSelect(job)}</div><div>${careerTime(job)}</div><div>${careerTags(job)}</div><div class="job-position-actions"><button type="button" class="career-tool" data-edit-job="${id}" aria-label="查看 ${wbEscape(job.company)}">${careerIcon('eye')}</button>${careerLink(job)}<button type="button" class="career-tool" data-delete-job="${id}" aria-label="删除 ${wbEscape(job.company)}">${careerIcon('trash')}</button></div></div>${careerWorkflow(job)}${careerClosure(job)}</article>`;
}
