...
function careerCard(job) {
 const id=wbEscape(job.id),meta=[job.city||job.location,job.jobType,job.companyType].filter(Boolean),times=applicationTimes(job);
 return `<article draggable="true" class="career-job application-card${job.status==='closed'?' is-closed':''}${careerPinned(job)?' is-pinned':''}" data-career-job="${id}"><header>${companyLogo(job)}<button class="application-company" type="button" data-edit-job="${id}">${wbEscape(job.company)}</button>${careerFavorite(job)}<button class="career-tool" type="button" data-application-menu="${id}" aria-label="更多操作">···</button></header><button type="button" class="application-role" data-edit-job="${id}">${wbEscape(job.role)}</button>${meta.length?`<p class="application-meta">${wbEscape(meta.join(' · '))}</p>`:''}${times.length?`<div class="application-times${times.some(t=>t.urgent)?' urgent':''}">${times.map(t=>applicationTimeButton(job,t.kind,t.date,t.label)).join('')}</div>`:''}${applicationTags(job)}<div class="application-shortcuts">${job.sourceUrl?`<button type="button" data-open-job-source="${wbEscape(job.sourceUrl)}">岗位链接 ↗</button>`:'<span></span>'}<button type="button" data-career-summary="${id}">投递总结</button></div><div class="application-advance"><span>${job.status==='closed'?'投递结果':'推进阶段'}</span>${applicationStage(job)}</div></article>`;
}
...
function careerTableRow(job) {
 ...
 return `<article class="job-position application-row${job.status==='closed'?' is-closed':''}${careerPinned(job)?' is-pinned':''}" data-job-id="${id}" tabindex="0" aria-label="${wbEscape(job.company)} ${wbEscape(job.role)}"><div class="job-position-main"><span><input type="checkbox" data-application-select="${id}" aria-label="选择 ${wbEscape(job.company)}"${applicationSelected.has(job.id)?' checked':''}></span><div class="application-company-cell">${companyLogo(job)}<div><button type="button" data-edit-job="${id}">${wbEscape(job.company)}</button><p>${wbEscape(job.role)} ${careerFavorite(job)}</p></div></div><div>${wbEscape(job.city||job.location||'—')}</div><div>${wbEscape(job.jobType||'—')}</div><div>${applicationTags(job)||'—'}</div><div>${applicationStage(job)}</div>${applicationProgress(job)}<div>${applicationTimeButton(job,'stage-deadline',D.deadline(job).value,D.deadline(job).label)}</div><div class="job-position-actions">${applicationActions(job)}</div></div></article>`;
}
