...
function careerTableRow(job) {
 ...
 return `<article class="job-position application-row${job.status==='closed'?' is-closed':''}${careerPinned(job)?' is-pinned':''}" data-job-id="${id}" tabindex="0" aria-label="${wbEscape(job.company)} ${wbEscape(job.role)}"><div class="job-position-main"><div class="application-company-cell">${companyLogo(job)}<div><button type="button" data-edit-job="${id}">${wbEscape(job.company)}</button><p>${wbEscape(job.role)} ${careerFavorite(job)}</p></div></div><div>${wbEscape(job.city||job.location||'—')}</div><div>${wbEscape(job.jobType||'—')}</div><div>${applicationTags(job)||'—'}</div><div>${applicationStage(job)}</div>${applicationProgress(job)}<div>${applicationTimeButton(job,'stage-deadline',D.deadline(job).value,D.deadline(job).label)}</div><div class="job-position-actions">${applicationActions(job)}</div></div></article>`;
}
