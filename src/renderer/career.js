...
function renderCareerOverview() {
  ...
  const compact=innerWidth>=1400&&innerHeight>=900, baseline=compact?75:150, amplitude=compact?60:110, chartHeight=compact?110:195;
  const max=Math.max(1,...data.trend.map(x=>x.count)), points=data.trend.map((x,i)=>`${35+i*85},${baseline-x.count/max*amplitude}`).join(' ');
  document.getElementById('careerTrend').innerHTML=`<div class="career-panel-head"><h2>本周投递趋势</h2><span>本周 ${data.trend.reduce((n,x)=>n+x.count,0)} 次</span></div><svg viewBox="0 0 580 ${chartHeight}" role="img" aria-label="周一至周日投递趋势"><path d="M35 ${baseline-amplitude}H545 M35 ${baseline-amplitude/2}H545 M35 ${baseline}H545" class="chart-grid"/><polyline points="${points}" class="chart-line"/>${data.trend.map((x,i)=>`<g tabindex="0" aria-label="${x.date}，${x.count} 次投递"><title>${x.date}：${x.count} 次投递</title><circle cx="${35+i*85}" cy="${baseline-x.count/max*amplitude}" r="5"/><text x="${35+i*85}" y="${chartHeight-8}" text-anchor="middle">${['周一','周二','周三','周四','周五','周六','周日'][i]}</text></g>`).join('')}</svg>`;
