...
Version 1.5.0 adds a local-first Job Radar with recommendation matching, public career-page sources, followed companies, a split-view job reader, preference controls, and a strict separation between discovered jobs and existing applications. Matching stays on the device, public pages are fetched only in the Electron main process, cached job descriptions remain outside the primary workspace JSON, and adding a recommendation creates a preparation-stage application without marking it as submitted.

Version 1.4.14 redesigns Quick Capture as an instant, keyboard-first Yanji glass surface with clearer item modes, a focused writing canvas, accessible state feedback, and compact shortcut guidance. Version 1.4.13 reduces duplicated workflow, IPC subscription, number-parsing, renderer test, and stylesheet code while preserving existing behavior and local data compatibility. Version 1.4.12 unifies scheduled tasks and calendar events without migrating existing data, combines linked records in the home-page Today Items view, merges quick-capture schedule and todo entry into Items, and adds safe backdrop closing for the note editor. Version 1.4.11 restores the v1.4.9 job-table rhythm while adding stable differential imports, separate JSON and image exports, selectable PNG/JPG previews, a restored job deadline column, and a one-day-ahead #1 full-screen deadline reminder. It also restores the packaged automatic updater dependency chain.
